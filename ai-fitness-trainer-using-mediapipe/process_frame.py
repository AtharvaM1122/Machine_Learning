...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.left_features['bicep_elbow'] = 13
        self.left_features['bicep_wrist'] = 15

        self.right_features['bicep_elbow'] = 14
        self.right_features['bicep_wrist'] = 16
...
if keypoints:  # Ensure keypoints are detected
    if keypoints.pose_landmarks:  # Properly indented inside `if keypoints`
        ...
        # Calculate elbow angles for bicep curl detection
        ...
        # Determine the state of the bicep curl
        if left_elbow_angle is not None:
            bicep_curl_state = self._get_bicep_curl_state(left_elbow_angle)
        elif right_elbow_angle is not None:
            bicep_curl_state = self._get_bicep_curl_state(right_elbow_angle)
        else:
            bicep_curl_state = None  # If no valid angle is found
...
# ------------------ BICEP CURL COUNTER ------------------
if bicep_curl_state == 'start' and 'end' in self.state_tracker['state_seq']:
    if not self.state_tracker['INCORRECT_POSTURE']:  # If posture is correct
        self.state_tracker['BICEP_CURL_COUNT'] += 1
        play_sound = str(self.state_tracker['BICEP_CURL_COUNT'])
    else:  # If posture is incorrect
        self.state_tracker['IMPROPER_CURL'] += 1
        play_sound = 'incorrect'

    self.state_tracker['state_seq'] = []  # Reset sequence after counting

elif bicep_curl_state == 'end':
    self.state_tracker['state_seq'].append('end')  # Add 'end' when arm is fully curled
            ...
            # Correct Squats
                ...
                draw_text(
                    frame, 
                    "CORRECT CURLS: " + str(self.state_tracker['BICEP_CURL_COUNT']), 
                    pos=(int(frame_width*0.68), 130),  # Below squat counters
                    text_color=(255, 255, 230),
                    font_scale=0.7,
                    text_color_bg=(0, 102, 255)
                )

                # Incorrect Bicep Curls
                draw_text(
                    frame, 
                    "INCORRECT CURLS: " + str(self.state_tracker['IMPROPER_CURL']), 
                    pos=(int(frame_width*0.68), 180),  # Below correct curls
                    text_color=(255, 255, 230),
                    font_scale=0.7,
                    text_color_bg=(255, 0, 0)  # Red for incorrect
                )  
